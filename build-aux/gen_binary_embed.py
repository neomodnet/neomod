#!/usr/bin/env python3
# Copyright (c) 2026, WH, All rights reserved.
# Generates source files and a header for embedding binary data in an executable.
#
# Usage:
#   gen_binary_embed.py -t <type> <output> <sym:file> [sym:file] ...
#   gen_binary_embed.py -t <type> <output> -f <manifest>
#
# Types:
#   asm-inline   GCC/Clang inline assembly with .incbin (auto-detects ELF/Mach-O/PE)
#   nasm         NASM assembly (for MSVC, outputs .asm instead of .cpp)
#   cppembed     C++23 #embed directive
#
# A header file (<output_stem>.h) is always generated alongside the source file,
# declaring each symbol's data and size (extern "C") plus the ALL_BINMAP lookup table.
#
# Manifest format (-f / -F):
#   Lines of "symbol:filepath", blank lines and #-comments are ignored.
#   -f manifests have --base-dir prepended to paths; -F manifests don't.

import sys
import os
import argparse
import hashlib

TYPES = ('asm-inline', 'nasm', 'cppembed')


def payload_fingerprint(entries):
    """Hash of every embedded payload, mixed into the generated source (see fingerprint_lines)."""
    digest = hashlib.sha256()
    for sym, filepath in entries:
        digest.update(sym.encode())
        try:
            with open(filepath, 'rb') as f:
                digest.update(f.read())
        except OSError:
            # let the compiler be the one to complain about a missing/unreadable payload
            digest.update(b'<unreadable>')
    return digest.hexdigest()[:16]


# #embed / .incbin pull their data in at compile time, but compiler caches (ccache, sccache, ...)
# key on the source *text*, and don't track those as dependencies - so editing an embedded asset
# without changing this generated file hands back a stale object from the cache, silently baking
# in the old data (ccache: fixed by PR #1765, broken in every release before that). mixing a hash
# of the payloads into the source keeps its text in sync with the data it embeds. it's emitted as
# a token rather than a comment so it survives preprocessing too (comments don't, and a cache
# that falls back to hashing preprocessed output would otherwise still go stale).
def fingerprint_lines(fingerprint, comment_prefix='//'):
    lines = [f'{comment_prefix} payload fingerprint: keeps this file\'s text in sync with the data it embeds,',
             f'{comment_prefix} so compiler caches can\'t hand back an object built from stale payloads']
    if comment_prefix == '//':
        lines.append(f'[[maybe_unused]] static constexpr char embed_payload_fingerprint[] = "{fingerprint}";')
    else:
        lines.append(f'%define EMBED_PAYLOAD_FINGERPRINT 0x{fingerprint}')
    lines.append('')
    return lines


def parse_entries(args_entries, manifest_files, build_manifest_files, base_dir):
    entries = []
    for manifest_file, apply_base in (
        *((m, True) for m in manifest_files),
        *((m, False) for m in build_manifest_files),
    ):
        with open(manifest_file, 'r') as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith('#'):
                    continue
                if ':' not in line:
                    print(
                        f'Error: manifest entry must be sym:file, got: {line}', file=sys.stderr)
                    sys.exit(1)
                sym, path = line.split(':', 1)
                path = path.strip()
                if apply_base and base_dir:
                    path = os.path.join(base_dir, path)
                entries.append((sym.strip(), path))
    for entry in args_entries:
        if ':' not in entry:
            print(
                f'Error: entry must be sym:file, got: {entry}', file=sys.stderr)
            sys.exit(1)
        sym, path = entry.split(':', 1)
        entries.append((sym, path))
    if not entries:
        print('Error: no symbol entries specified', file=sys.stderr)
        sys.exit(1)
    return entries


# every scheme exports the payload as `const unsigned char sym[]` and its byte count as
# `const unsigned long long sym_size`, emitted right next to the data by whatever embeds it
# (sizeof for #embed, an assembler-computed constant for .incbin). taking the size in c++ as
# the distance to an exported `sym_end` label instead is pointer arithmetic between two
# objects (UB), and with #embed asan's redzones actually make it wrong.
def gen_header(entries):
    lines = [
        '// Auto-generated by gen_binary_embed.py - do not edit',
        '#pragma once',
        '',
        '#include <string_view>',
        '#include <unordered_map>',
        '',
        'extern "C" {',
    ]
    for sym, _ in entries:
        lines.append(f'extern const unsigned char {sym}[];')
        lines.append(f'extern const unsigned long long {sym}_size;')
    lines.append('} // extern "C"')
    lines.append('')
    lines.append(
        'inline const std::unordered_map<std::string_view, std::string_view> ALL_BINMAP{')
    if len(entries) > 0:
        for sym, _ in entries:
            lines.append(f'{{"{sym}", {{reinterpret_cast<const char*>(&{sym}[0]), static_cast<size_t>({sym}_size)}}}},')
        lines[-1] = lines[-1][:-1]
    lines.append('};')
    lines.append('')
    return '\n'.join(lines)


def gen_cppembed(entries, output_dir, fingerprint):
    lines = [
        '// Auto-generated by gen_binary_embed.py - do not edit',
        '#include <cstddef>',
        '',
        *fingerprint_lines(fingerprint),
        'extern "C" {',
    ]
    for sym, filepath in entries:
        # #embed resolves a quoted path relative to this generated file's own dir
        # (like #include ""), not the build dir, so rewrite the build-dir-relative
        # manifest path accordingly. forward slashes work for every toolchain.
        try:
            embed_path = os.path.relpath(filepath, output_dir)
        except ValueError:  # paths on different drives (windows)
            embed_path = filepath
        embed_path = embed_path.replace(os.sep, '/')
        lines.append('')
        lines.append(f'extern const unsigned char {sym}[] = {{')
        lines.append(f'#embed "{embed_path}"')
        lines.append('};')
        lines.append(f'extern const unsigned long long {sym}_size = sizeof({sym});')
    lines.append('')
    lines.append('} // extern "C"')
    lines.append('')
    return '\n'.join(lines)


def gen_asm_inline(entries, fingerprint):
    lines = [
        '// Auto-generated by gen_binary_embed.py - do not edit',
        '',
        *fingerprint_lines(fingerprint),
        '#if defined(__APPLE__) && defined(__MACH__)  // Mach-O',
        '',
    ]
    for sym, filepath in entries:
        lines.append('__asm__(')
        lines.append('    ".section __DATA,__const\\n"')
        lines.append('    ".balign 1\\n"')
        lines.append(f'    ".globl _{sym}\\n"')
        lines.append(f'    "_{sym}:\\n"')
        lines.append(f'    ".incbin \\"{filepath}\\"\\n"')
        lines.append(f'    "L_{sym}_end:\\n"')
        lines.append('    ".balign 8\\n"')
        lines.append(f'    ".globl _{sym}_size\\n"')
        lines.append(f'    "_{sym}_size:\\n"')
        lines.append(f'    ".quad L_{sym}_end - _{sym}\\n"')
        lines.append('    ".section __TEXT,__text\\n"')
        lines.append(');')
        lines.append('')
    lines.append('#elif defined(_WIN32)  // MinGW PE/COFF')
    lines.append('')
    lines.append('#ifdef _WIN64')
    lines.append('#define _INCBIN_PFX ""')
    lines.append('#else')
    lines.append('#define _INCBIN_PFX "_"')
    lines.append('#endif')
    lines.append('')
    for sym, filepath in entries:
        lines.append('__asm__(')
        lines.append('    ".section .rdata,\\"dr\\"\\n"')
        lines.append('    ".balign 1\\n"')
        lines.append(f'    ".globl " _INCBIN_PFX "{sym}\\n"')
        lines.append(f'    _INCBIN_PFX "{sym}:\\n"')
        lines.append(f'    ".incbin \\"{filepath}\\"\\n"')
        lines.append(f'    ".L{sym}_end:\\n"')
        lines.append('    ".balign 8\\n"')
        lines.append(f'    ".globl " _INCBIN_PFX "{sym}_size\\n"')
        lines.append(f'    _INCBIN_PFX "{sym}_size:\\n"')
        lines.append(f'    ".quad .L{sym}_end - " _INCBIN_PFX "{sym}\\n"')
        lines.append('    ".section .text\\n"')
        lines.append(');')
        lines.append('')
    lines.append('#undef _INCBIN_PFX')
    lines.append('')
    lines.append('#else  // ELF')
    lines.append('')
    for sym, filepath in entries:
        lines.append('__asm__(')
        lines.append('    ".section .rodata\\n"')
        lines.append('    ".balign 1\\n"')
        lines.append(f'    ".globl {sym}\\n"')
        lines.append(f'    ".type {sym}, @object\\n"')
        lines.append(f'    "{sym}:\\n"')
        lines.append(f'    ".incbin \\"{filepath}\\"\\n"')
        lines.append(f'    ".L{sym}_end:\\n"')
        lines.append(f'    ".size {sym}, .L{sym}_end - {sym}\\n"')
        lines.append('    ".balign 8\\n"')
        lines.append(f'    ".globl {sym}_size\\n"')
        lines.append(f'    ".type {sym}_size, @object\\n"')
        lines.append(f'    ".size {sym}_size, 8\\n"')
        lines.append(f'    "{sym}_size:\\n"')
        lines.append(f'    ".quad .L{sym}_end - {sym}\\n"')
        lines.append('    ".section \\".text\\"\\n"')
        lines.append(');')
        lines.append('')
    lines.append('#endif')
    lines.append('')
    return '\n'.join(lines)


def gen_nasm(entries, fingerprint):
    lines = [
        '; Auto-generated by gen_binary_embed.py - do not edit',
        *fingerprint_lines(fingerprint, ';'),
        'section .data',
    ]
    for sym, filepath in entries:
        lines.append(f'global {sym}')
        lines.append(f'global {sym}_size')
        lines.append(f'{sym}:')
        lines.append(f'    incbin "{filepath}"')
        lines.append('.end:')
        lines.append('    align 8, db 0')
        lines.append(f'{sym}_size: dq {sym}.end - {sym}')
    lines.append('')
    return '\n'.join(lines)


def write_if_changed(path, content):
    if os.path.exists(path):
        try:
            with open(path, 'r') as f:
                if f.read() == content:
                    return False
        except:
            pass
    os.makedirs(os.path.dirname(path) or '.', exist_ok=True)
    with open(path, 'w') as f:
        f.write(content)
    return True


def main():
    parser = argparse.ArgumentParser(
        description='Generate source files for embedding binary data in executables.')
    parser.add_argument('-t', '--type', required=True, choices=TYPES,
                        help='Output type: asm-inline, nasm, or cppembed')
    parser.add_argument('-f', '--manifest', metavar='FILE', action='append', default=[],
                        help='Read symbol:filepath entries from a manifest file (can be repeated)')
    parser.add_argument('-F', '--build-manifest', metavar='FILE', action='append', default=[],
                        help='Like -f, but paths are relative to CWD (--base-dir is not applied)')
    parser.add_argument('-b', '--base-dir', default='',
                        help='Base directory to prepend to -f manifest file paths (not -F or CLI entries)')
    parser.add_argument('-d', '--depfile', metavar='FILE',
                        help='Write a Make-format dependency file listing all embedded files')
    parser.add_argument(
        'output', help='Output source file path (.cpp or .asm)')
    parser.add_argument('entries', nargs='*', metavar='sym:file',
                        help='Symbol-to-file mappings')

    args = parser.parse_args()
    entries = parse_entries(args.entries, args.manifest, args.build_manifest, args.base_dir)

    generators = {
        'asm-inline': gen_asm_inline,
        'nasm': gen_nasm,
    }
    # cppembed needs paths relative to the generated file's own dir (see gen_cppembed);
    # .incbin (asm-inline/nasm) resolves relative to the build dir (cwd), so it keeps
    # the build-dir-relative manifest paths as-is.
    is_cppembed = args.type == 'cppembed'
    fingerprint = payload_fingerprint(entries)
    if is_cppembed:
        source = gen_cppembed(entries, os.path.dirname(args.output) or '.', fingerprint)
    else:
        source = generators[args.type](entries, fingerprint)

    header = gen_header(entries)

    stem, _ = os.path.splitext(args.output)
    header_path = stem + '.h'

    # always write the source file so Make sees a fresh timestamp and recompiles
    # the .o (which picks up embedded file contents via .incbin / #embed). the recompile
    # only actually reads the payloads again because the fingerprint changed with them,
    # see fingerprint_lines()
    os.makedirs(os.path.dirname(args.output) or '.', exist_ok=True)
    with open(args.output, 'w') as f:
        f.write(source)
    # header uses write_if_changed to avoid cascading recompilation
    write_if_changed(header_path, header)

    if args.depfile:
        deps = [filepath for _, filepath in entries] + args.manifest + args.build_manifest
        depfile_content = f'{args.output}: \\\n'
        depfile_content += ' \\\n'.join(f'  {d}' for d in deps)
        depfile_content += '\n'
        # emit a phony target per prerequisite (gcc -MP style) so that a later-deleted or renamed
        # embedded file/manifest doesn't break the build with "No rule to make target" before the
        # depfile can be regenerated
        for d in deps:
            depfile_content += f'{d}:\n'
        write_if_changed(args.depfile, depfile_content)


if __name__ == '__main__':
    main()
